...
ErlangExp_Fit_v2_Pvalue <- function(empiricaldata, k_star, erlambda_star, explambda_star,
  ...
  # 2. Theoretical ErlangExp CDF at each sorted data point: F_i = F(x_(i))
  params <- c(k_star, erlambda_star, explambda_star)
  Fi <- ErlangExpCDF_Func(params, x_vals, interval = 0.01)
  ...
  for (i in 1:n) {
    ...
    Fi_sample <- ErlangExpCDF_Func(params, x_sample, interval = 0.01)
