...
![Comparison of waiting time distributions in the exposed compartment (E) of an SEIR model. Left: compartment structure. Middle: resulting waiting time distribution. Right: corresponding ODEs describing transitions out of E.](figures/lct_diagram_v2.png){width=100%}
...
The Erlang distribution is one of many **phase-type distributions** that can be implemented via the Linear Chain Trick, including hypoexponential, hyper-Erlang, and Coxian distributions (see [Hurtado & Kirosingh 2019](https://doi.org/10.1007/s00285-019-01412-w) for details). 
...
1. **What value of K** (number of sub-compartments) best represents your data?
2. **What rate parameter(s)** should you use in your ODE model?

`GenErlangFit` answers both questions by fitting Erlang and Erlang-Exponential distributions to your data and using AIC-based model selection to identify the optimal K.
